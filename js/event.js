function Event(eventJson_,parentScene_){
	...
	this.display=function(this_){//how do I use this also in delayed function
		...
		if(self.content.type=="text"){
			self.htmlElement = document.createElement("span");
			self.htmlElement.innerHTML=self.content.value;
			document.getElementById("main_text").append(self.htmlElement);

		}else if(self.content.type=="text-clickable"){
			self.htmlElement = document.createElement("a");

			self.htmlElement.onclick="return false"

			self.htmlElement.innerHTML=self.content.value;
			//self.htmlElement.innerHTML='<a href="" onclick="return false;" id="nothing"">' + self.content.value+ '</a>';
			document.getElementById("main_text").append(self.htmlElement);
	...
	this.linkTargets=function(){
		for(let i=0;i<this.targets.length;i++){
			...
			if(!isNaN(this.targets[i].id)){//if target is numbers it is an event if it is letters it is a new scene // returns true if it starts with numbers
				...
				this.targets[i].object=targetEvent;
	...
	for(let i=0;i<this.targets.length;i++){
		...
		if(!isNaN(this.targets[i].id)){//if target is numbers it is an event if it is letters it is a new scene // returns true if it starts with numbers
			...
			if(this.targets[i].type=="time"){//if time trigger
				...
				// orderedListStacking[]
				//let targetEvent=this.getTargetEvent(this.targets[i].id);
				let targetEvent=this.targets[i].object
				...
				if(targetEvent != null){
					...
					let clickableObject=this.htmlElement;

					//document.getElementById(this.parentScene.code+'_'+this.id);
					//console.log(clickableObject)
					clickableObject.targetEvent=this.targets[i].object;
